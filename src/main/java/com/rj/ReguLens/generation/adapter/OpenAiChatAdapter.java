...
@Component("openAiChatAdapter")
@Primary
@ConditionalOnProperty(name = "regulens.ai.llm.provider", havingValue = "openai")
@Slf4j
public class OpenAiChatAdapter implements LlmClientPort {
    ...
    public OpenAiChatAdapter(
            @Value("${regulens.ai.llm.api-key:${OPENAI_API_KEY:}}") String apiKey,
            @Value("${regulens.ai.llm.model-name:gpt-4o-mini}") String modelName,
        ...
        if (apiKey != null && !apiKey.isBlank() && !apiKey.equals("test-key")) {
            log.info("Initializing OpenAiChatModel with model: {}", this.modelName);
            ...
            log.warn("OpenAI API key is not configured. Operating in fallback mock chat mode.");
            this.chatModel = null;
        }
    }
    ...
    public LlmResponse generate(LlmRequest request) {
        ...
        try {
            ...
            log.error("Error during OpenAI LLM generation call, falling back to mock adapter: {}", e.getMessage());
            return fallbackAdapter.generate(request);
        }
    }
